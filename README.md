...
| [`review-feedback`](skills/review-feedback) | Write the comments a reviewer would post on a code pull request, in their stance: diff before description, questions where the answer is the author's, placement and dependency direction, naming, prose, each comment marked question / request / nit and deferrable or not, praise kept separate. Runs `review-pr` after its own reading when installed. Reads the reviewer's past comments from `.claude/review-feedback/examples.md` when present. | Own, MIT. Code pull requests only. |
...
| [`databricks-core`](skills/databricks-core) | Databricks CLI operations: authentication, profile selection, bundles, and manual data exploration. Entry point for the other `databricks-*` skills; install it with any of them. | Vendored from [databricks/databricks-agent-skills](https://github.com/databricks/databricks-agent-skills/tree/main/skills/databricks-core) at `0fbf9ba` (v0.2.18), Databricks License (see its `LICENSE.txt` and `NOTICE.txt`). |
| [`databricks-data-discovery`](skills/databricks-data-discovery) | Find and explore Databricks data through Genie from the CLI: which tables exist, where a dataset lives, natural-language questions over the data, SQL generation. | Vendored from [databricks/databricks-agent-skills](https://github.com/databricks/databricks-agent-skills/tree/main/skills/databricks-data-discovery) at `0fbf9ba` (v0.2.18), Databricks License (see its `LICENSE.txt` and `NOTICE.txt`). |
| [`databricks-dbsql`](skills/databricks-dbsql) | Databricks SQL: SQL warehouses, SQL scripting and stored procedures, materialized views, pipe syntax, geospatial and collation functions, AI functions, federation. | Vendored from [databricks/databricks-agent-skills](https://github.com/databricks/databricks-agent-skills/tree/main/skills/databricks-dbsql) at `0fbf9ba` (v0.2.18), Databricks License (see its `LICENSE.txt` and `NOTICE.txt`). |
| [`databricks-unity-catalog`](skills/databricks-unity-catalog) | Unity Catalog governance: GRANT/REVOKE and the privilege model, row filters and column masks, external locations, securable DDL, system tables, volumes. | Vendored from [databricks/databricks-agent-skills](https://github.com/databricks/databricks-agent-skills/tree/main/skills/databricks-unity-catalog) at `0fbf9ba` (v0.2.18), Databricks License (see its `LICENSE.txt` and `NOTICE.txt`). |
| [`databricks-dabs`](skills/databricks-dabs) | Declarative Automation Bundles (formerly Databricks Asset Bundles): create, validate, deploy, and run bundles for jobs, pipelines, dashboards, alerts, volumes, and apps. | Vendored from [databricks/databricks-agent-skills](https://github.com/databricks/databricks-agent-skills/tree/main/skills/databricks-dabs) at `0fbf9ba` (v0.2.18), Databricks License (see its `LICENSE.txt` and `NOTICE.txt`). |
| [`databricks-jobs`](skills/databricks-jobs) | Lakeflow Jobs: task types, triggers and schedules, notifications, deployment through bundles, the Python SDK, or the CLI. | Vendored from [databricks/databricks-agent-skills](https://github.com/databricks/databricks-agent-skills/tree/main/skills/databricks-jobs) at `0fbf9ba` (v0.2.18), Databricks License (see its `LICENSE.txt` and `NOTICE.txt`). |
| [`databricks-python-sdk`](skills/databricks-python-sdk) | Databricks Python SDK, Databricks Connect, and REST API usage with runnable examples. | Vendored from [databricks/databricks-agent-skills](https://github.com/databricks/databricks-agent-skills/tree/main/skills/databricks-python-sdk) at `0fbf9ba` (v0.2.18), Databricks License (see its `LICENSE.txt` and `NOTICE.txt`). Body exceeds 500 lines; kept as upstream ships it. |
| [`databricks-docs`](skills/databricks-docs) | Databricks documentation lookup through the llms.txt index, for topics the other skills do not cover. | Vendored from [databricks/databricks-agent-skills](https://github.com/databricks/databricks-agent-skills/tree/main/skills/databricks-docs) at `0fbf9ba` (v0.2.18), Databricks License (see its `LICENSE.txt` and `NOTICE.txt`). |
...
gh skill install yamat47/github-toolkit review-feedback
```

`skill-creator`, `frontend-design`, and `gh-stack` are unmodified upstream copies except for the `license:` frontmatter line, kept here so one `gh skill install` source covers everything. The `databricks-*` skills are upstream copies with two frontmatter changes: a `license:` line was added, and the upstream `parent:` key was moved under `metadata:` because the Agent Skills validator rejects unknown top-level keys. Their bodies are unmodified. They are distributed under the [Databricks License](skills/databricks-core/LICENSE.txt), which is not an open-source license: it permits use and redistribution only in connection with the Databricks services, and Databricks may terminate it. The same skills are available first-party through `databricks aitools install`; install from one source or the other, not both. Skills that take a Claude Code-only frontmatter key such as `user-invocable` do not carry it, because the Agent Skills validator rejects unknown keys; their descriptions say when they are background knowledge instead.
...
To vendor a skill from another repository instead, run `tools/vendor-skill.sh OWNER/REPO PATH REF`, keep its license file, add `license:` to the frontmatter if upstream lacks it, and record the upstream commit in the Skills table. Re-run the script with a newer ref to refresh. The `databricks-*` skills are refreshed together from one upstream tag; copy the upstream root `LICENSE` and `NOTICE` into each directory as `LICENSE.txt` and `NOTICE.txt`, and re-apply the frontmatter changes described under Skills.
